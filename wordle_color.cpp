...
colored tiles, indicating when letters match or occupy the correct position. Yellow highlighted characters
indicate that the letter is in the solution but not in that position. Green highlighted characters
indicate that the letter is in the correct location of the solution. Grey highlighted characters indicate
that the letter is not in the solution at all.
*/

// Compile command: g++ -std=c++20 -o wordle_color wordle_color.cpp
// Run command: ./wordle_color
...
void displayGame (string display[][6], int row_size, string target, string input, int hint_index);
...
void verifyInput (string target, string input, string word_array[]) {
    ...
    string guess_display[ALLOWED_GUESSES][6] = {{"-", "-", "-", "-", "-", "\n"}, {"-", "-", "-", "-", "-", "\n"}, {"-", "-", "-", "-", "-", "\n"}, {"-", "-", "-", "-", "-", "\n"}, {"-", "-", "-", "-", "-", "\n"}, {"-", "-", "-", "-", "-", "\n"}};

    int number_of_guesses = 0; //Counter for the number of guesses
    int hint_display_index = 0; //Counter for the index in the array
    ...
    do {
        ...
        displayGame (guess_display, ALLOWED_GUESSES, target, input, hint_display_index);
        hint_display_index += 1;
    }
...
void runGame (string word_array[]) {
   ...
   // This portion allows for the user to play again or quit the game
    ...
    do {
        ...
        // cout << "Mystery word: " << mystery_word << endl;
...
void displayGame (string display[][6], int row_size, string target, string input, int hint_index) {
    ...
    for (int i = 0; i < 5; i++) {
        //Correct letter mechanic and correct letter and position mechanic
        for (int j = 0; j < 5; j++) {

            string input_index(1, input[j]);

            if (input[j] == target[i] && (j != i)) {
                display[hint_index][j] = "\033[;33m" + input_index + "\033[0m";
            }
            if ((input[j] == target[i]) && (j == i)) {
                display[hint_index][j] = "\033[;32m" + input_index + "\033[0m";
            }

        }

        //Incorrect letter and position mechanic
        for (int k = 0; k < 5; k++) {
            string input_index(1, input[k]);
            if (display[hint_index][k] == "-") {
                display[hint_index][k] = "\033[;31m" + input_index + "\033[0m";
            }
        }
    }
    
    //Display player guesses mechanic
